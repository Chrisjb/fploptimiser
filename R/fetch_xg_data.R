...
fetch_xg_data <- function(year = 2023, reduce=FALSE, check_data=FALSE){
  ...
  team_names_common <- data.frame(fpl_name =unique(fpl_dat$name)) %>%
    mutate(understat_name = stringr::str_replace_all(fpl_name, ' ','_'),
           ...
           understat_name =stringr::str_replace(understat_name, '(Newcastle)','\\1_United'),
           ...
           understat_name =dplyr::if_else(understat_name == "Nott'm_Forest", "Nottingham_Forest", understat_name)
           )

  # for debugging:
  # check all understat names are mapped correctly from the fpl name
  # all(understat$team_name %in% team_names_common$understat_name)
  ...
  found8 <- fpl_dat9_unmatched %>%
    select(-understat_games, -understat_minutes, -understat_goals, -xG, -xA, -understat_assists, -understat_shots, -understat_key_passes, -npg, -npxG, -team_code) %>%
    mutate(web_name2 = stringr::str_to_upper(stringr::str_extract(web_name, '(?<=\\.).+'))) %>%
    left_join(understat_surnames, by=c('web_name2' = 'understat_surname')) %>%
    filter(!is.na(xG)) %>%
    select(-web_name2)

  fpl_dat10 <- fpl_dat9 %>%
    filter(!web_name %in% found8$web_name) %>%
    bind_rows(found8)


  # unmatched args should only be players that haven't played this season (and thus not in understat data)
  fpl_dat10_unmatched  <- fpl_dat10 %>%
    filter(is.na(xG)) %>%
    filter(minutes >0) %>%
